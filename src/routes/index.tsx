...
function Gallery() {
  const reduced = !!useReducedMotion();
  const photos = [
    { src: clinicImg, alt: "Fenan Dental Clinic interior", cols: "md:col-span-2 md:row-span-2" },
    { src: toolsImg, alt: "Precision dental instruments", cols: "" },
    { src: smileImg, alt: "Patient with a healthy smile", cols: "" },
  ];
  return (
    <section className="bg-background px-6 py-32">
      <div className="mx-auto max-w-7xl">
        <div className="mb-12 flex flex-wrap items-end justify-between gap-6">
          <div>
            <motion.p initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={reduced ? {} : { type: "spring", stiffness: 280, damping: 26 }} className="mb-3 text-xs tracking-[0.3em] text-teal">INSIDE THE CLINIC</motion.p>
            <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={reduced ? {} : { type: "spring", stiffness: 260, damping: 26, delay: 0.06 }} className="font-display text-4xl font-bold text-ink md:text-5xl">
              Crafted spaces,<br />trusted tools.
            </motion.h2>
          </div>
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={reduced ? {} : { delay: 0.16 }} className="max-w-xs text-sm leading-relaxed text-muted-foreground">
            A calming environment paired with precision instruments — designed to put every patient at ease from the moment they arrive.
          </motion.p>
        </div>
        <div className="grid gap-4 md:grid-cols-3" style={{ gridAutoRows: "240px" }}>
          {photos.map((p, i) => (
            <motion.div key={p.src} initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }}
              transition={reduced ? {} : { type: "spring", stiffness: 220, damping: 24, delay: i * 0.09 }}
              className={`overflow-hidden rounded-2xl ${p.cols}`}>
              <img src={p.src} alt={p.alt} loading="lazy" className="h-full w-full object-cover transition duration-500 hover:scale-[1.04]" />
            </motion.div>
          ))}
        </div>
      </div>
    ...
    <section id="contact" className="bg-ink-gradient px-6 py-32 text-white">
      <div className="mx-auto max-w-5xl text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={sp(0.28)} className="flex flex-wrap justify-center gap-4">
          ...
          <MagnetWrap><a href={SITE.whatsapp.url} target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/25 px-9 py-4 text-sm font-medium text-white transition hover:bg-white/10">WhatsApp Us</a></MagnetWrap>
